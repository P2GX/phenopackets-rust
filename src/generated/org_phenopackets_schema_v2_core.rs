...
/// We choose not to output empty strings for optional elements in the Phenopacket Schema.
#[cfg(feature = "serde")]
impl Serialize for ExternalReference {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where S: Serializer {
        let mut extref = serializer.serialize_struct("ExternalReference", 3)?;
        if ! self.id.is_empty() {
            extref.serialize_field("id", &self.id)?;
        }
        if ! self.reference.is_empty() {
            extref.serialize_field("reference", &self.reference)?;
        }
        if ! self.description.is_empty() {
            extref.serialize_field("description", &self.description)?;
        }
        extref.end()
    }
}
...
impl Serialize for Evidence {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where S: Serializer {
        let mut evidence = serializer.serialize_struct("Evidence", 2)?;
        match &self.evidence_code {
            Some(ecode) => {
                evidence.serialize_field("evidence_code", ecode)?;
            },
            None => { /* no op, this is not required */}
        }
        match &self.reference {
            Some(reference) => {
                // only serialize if at least one field is present
                if ! reference.description.is_empty() || !reference.id.is_empty() || !reference.reference.is_empty() {
                    evidence.serialize_field("reference", reference)?;
                }
            },
            None => { /* no op, this is not required */}
        }
